...
#: сайт отдал расписание на ближние дни, а на дальний не дал ничего: у него
#: программа просто короче нашего окна (`rtp.pt` RTP Açores — четыре дня,
#: остальные каналы RTP дают неделю). Это не поломка, и в «Сломанные» такой
#: сайт попадать не должен — владелец 23.09.2026
SHORT_DEPTH = "нет на этот день"


def soften_short_depth(rows: list[dict]) -> int:
    """Страница одного дня отвечает «не найдено», а сам сайт жив — это не
    поломка, а отсутствие программы за этот день. Смягчаем вердикт.

    Условие строгое: сайт в этом же прогоне отдал расписание на других
    страницах, и отказ именно «не найдено» (404), а не отказ пускать,
    таймаут или заглушка защиты. Такую строку видно в отчёте отдельно, но
    сайт из-за неё не числится сломанным (владелец 23.09.2026, RTP Açores:
    программа на 5 дней, остальные каналы RTP дают неделю).
    """
    alive = {r.get("domain") for r in rows
             if r.get("итог") == "расписание есть"}
    softened = 0
    for r in rows:
        if r.get("итог") != "не открылась" or r.get("domain") not in alive:
            continue
        why = str(r.get("почему") or "")
        if "404" not in why or "защит" in why.lower():
            continue          # не «нет страницы», а настоящий отказ
        r["итог"] = SHORT_DEPTH
        r["почему"] = "сайт не публикует программу за этот день — не поломка"
        softened += 1
    return softened
...
def main() -> int:
    ...
    # сайт с короткой программой не сломан: смягчаем до оценки провала,
    # иначе он потянет обход в «красные» и займёт место в «Сломанных»
    softened = soften_short_depth(report)
    if softened:
        print(f"дальние дни без программы (не поломка): {softened} стр.")
